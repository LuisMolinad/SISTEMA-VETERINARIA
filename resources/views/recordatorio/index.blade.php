...
@section('librerias')
    ...
    <script src=" {{ asset('js/Recordatorio/recordatorio_enviar_mensajes.js') }} "></script>
...
@section('content')
    ...
    <div class="table-responsive-sm container-fluid contenedor">
            ...
            <div class="banner_03">
                @can('enviar-Recordatorio')
                    <div class="boton crear container_btn_hend">
                        <a href="/recordatorio/enviar_ui"><button type="button" class="btn btn-success boton_crear">Crear
                                mensaje de recordatorio</button></a>
                    </div>
                @endcan
            </div>
        </div>
                            ...
                            ?>>
                                ...
                                @if ($recordatorio->estado == 0)
                                    <td class="botones-linea-xl">
                                        <a href="{{ url('/recordatorio/' . $recordatorio->id) }}"><button type="button"
                                                class="btn btn-primary">Consultar</button></a>

                                        @can('editar-Recordatorio')
                                            <a href="{{ url('/recordatorio/' . $recordatorio->id . '/edit') }}"><button
                                                    type="button" class="btn btn-warning">Editar</button></a>
                                        @endcan

                                        @can('borrar-Recordatorio')
                                            <form id="EditForm{{ $recordatorio->id }}"
                                                action="{{ url('/recordatorio/' . $recordatorio->id) }}" method="post">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                                <button
                                                    onclick="return alerta_eliminar_recordatorio('{{ $recordatorio->id }}');"
                                    ...
                                    <td id="botones-linea">
                                        ...
                                        @can('borrar-Recordatorio')
                                            <form class="candidatos_a_eliminar"
                                                action="{{ url('/recordatorio/' . $recordatorio->id) }}" method="post">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                    ...
                                    <td id="botones-linea">
                                        ...
                                        <a href="{{ url('/recordatorio/reenviar/' . $recordatorio->id) }}"><button
                                                type="button" class="btn btn-info">Reenviar</button></a>
                                @endif
                                </td>
        ...
        <div class="boton crear container_btn_hstart">
            ...
            @can('borrar-Recordatorio')
                {!! Form::open([
                    'url' => '/recordatorio/eliminar_masivo',
                    'method' => 'GET',
                ]) !!}
                {!! Form::submit('Eliminar mensajes enviados', [
                    'class' => 'btn btn-danger boton_opcional',
                ]) !!}
                {!! Form::close() !!}
            @endcan
