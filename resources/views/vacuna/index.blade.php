                ...
                <tbody>
                    ...
                    >
                        ...
                        <td class="botones-linea-xl">
                            ...
                            <a href="{{ route('vacuna.show',$vacuna->id) }}" style="margin-right: 10px"><button type="button" class="btn btn-info basis1" >Consultar</button></a>   
                            <a href="{{ url('/vacuna/'.$vacuna->id.'/edit') }}" style="margin-right: 10px"><button type="button" class="btn btn-warning basis2" >Editar</button></a>
                            @if($vacuna->disponibilidadVacuna == True)
                                <form id="DeshabilitarForm{{$vacuna->id}}" action="{{route('Vacuna.update',[$vacuna->id,'accion'=>"deshabilitar"])}}" method="post">
                                    ...
                                    <button onclick="return alerta_deshabilitar_vacuna('{{$vacuna->nombreVacuna}}','{{$vacuna->id}}');" type="submit" class="btn btn-secondary basis3" style="margin-right: 10px">Deshabilitar</button>
                                </form>
                            @else
                                <form id="HabilitarForm{{$vacuna->id}}" action="{{route('Vacuna.update',[$vacuna->id,'accion'=>"habilitar"])}}" method="post">                            
                                    ...
                                    <button onclick="return alerta_habilitar_vacuna('{{$vacuna->nombreVacuna}}','{{$vacuna->id}}');" type="submit" class="btn btn-secondary basis3" style="width:110px;margin-right: 10px">Habilitar</button>
                                </form>
                            @endif
                            <form id="BorrarForm{{$vacuna->id}}" action="{{url('/vacuna/'.$vacuna->id)}}" method="post">
                                @csrf
                                {{method_field('DELETE')}}
                                <button onclick="return alerta_eliminar_vacuna('{{$vacuna->nombreVacuna}}','{{$vacuna->id}}');" type="submit" class="btn btn-danger basis4">Eliminar</button>
                            </form>
                        </td>
