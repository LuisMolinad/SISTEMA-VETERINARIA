                                ...
                                <tbody>
                                    @foreach ($datos['record'] as $record)
                                        @if ($vacuna->id === $record->vacuna_id)
                                            <tr>
                                                <td id_linea="{{ $record->id }}" class="record-fecha">
                                                    {{ $record->fecha }}</td>
                                                <td id_linea="{{ $record->id }}" class="record-peso">{{ $record->peso }}
                                                </td>
                                                <td id_linea="{{ $record->id }}" class="record-refuerzo">
                                                    {{ $record->refuerzo }}</td>
                                                <td>
                                                    @can('borrar-RecordVacunacion')
                                                        <form action="{{ url('/record/' . $record->id) }}" method="post"
                                                            ...
                                                            <button type="submit" class="btn btn-danger"
                                                                onclick="return alerta_eliminar_record('{{ $record->id }}')"><img
                                                                    src="{{ asset('svg/trash-can.svg') }}" width="20"
                                                                    alt="Eliminar"></button>
                                                        </form>
                                                    @endcan
                                                </td>
