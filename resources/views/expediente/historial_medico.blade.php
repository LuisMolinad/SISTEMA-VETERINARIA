...
@section('header')
    <h1 class="header">Historial de la Mascota {{ $expediente->mascota->nombreMascota }} - ID Mascota:
        {{ $expediente->mascota->idMascota }}</h1>
@endsection
    ...
    <div class="table-responsive-sm container-fluid contenedor">
        ...
        @if ($expediente->mascota->fallecidoMascota == 'Vivo')
            <form method="GET" action="/historial_medico/fetch" id="agregar_linea" class="needs-validation" novalidate>
                @can('crear-LineaHistorial')
                    <input type="text" id="expediente_id" name="expediente_id" value="{{ $expediente->id }}" class="none">
                    <div class="col-lg-12">
                        <br><label for=""
                            style="margin-bottom: 10px; font-size: 22px; font-weight: bold;">Diagnostico:</label><br>
                        <!--style="resize: both;-->
                <textarea id="textoLineaHistorial" type="text"
                            name="textoLineaHistorial" class="form-control" rows="3"
                            placeholder='Escriba el diagnostico de la mascota' required></textarea>
                    ...
                    <div class="col-lg-12">
                        ...
                        <button onclick="return agregar_linea()" class="btn btn-success float-right"
                            style="margin-top: 10px">Agregar</button>
            ...
            <tbody>
                ...
                @foreach ($expediente->HistorialMedico as $lineaHistorial)
                    <tr>
                        ...
                        <td class="linea" id_linea="{{ $lineaHistorial->id }}">{{ $lineaHistorial->textoLineaHistorial }}
                        </td>
                        <td>
                            <form id="EditForm{{ $lineaHistorial->id }}"
                                ...
                                {{ method_field('DELETE') }}
                                @can('borrar-LineaHistorial')
                                    <button onclick="return alerta_eliminar_lineahistorial('{{ $lineaHistorial->id }}')"
